...
// Middleware
app.use(cors({
    origin: 'http://localhost:3000', // e.g., http://192.168.1.100:3000
    credentials: true
}));
...
const isAuthenticated = (req, res, next) => {
    if (req.session.user) {
        ...
        res.redirect('http://localhost:3000/Login/index.html'); // e.g., http://192.168.1.100:3000/Login/index.html
    }
};
...
app.post('/api/biometric-auth', (req, res) => {
    ...
    setTimeout(() => {
        ...
        res.status(200).json({ 
            ...
            redirect: 'http://localhost:3000/Dashboard/index.html' // e.g., http://192.168.1.100:3000/Dashboard/index.html
        });
    }, 2000);
});
...
app.post('/api/logout', (req, res) => {
    req.session.destroy(err => {
        ...
        res.status(200).json({ 
            ...
            redirect: 'http://localhost:3000/Login/index.html' // e.g., http://192.168.1.100:3000/Login/index.html
        });
    });
});
...
// Start server
app.listen(port, () => {
    console.log(`Server running at http://localhost:3000`); // e.g., http://192.168.1.100:3000
});